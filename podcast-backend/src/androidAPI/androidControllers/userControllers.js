...
const createChannel = async (req, res) => {
  try {
    ...
    const createChannel = await User.updateOne(
      { _id: userId },
    ...
    if (createChannel) {
      return res.status(201).json({ status: "success", data: createChannel });
    } else {
      return res
        .status(500)
        .json({ status: "failed", message: "channel creating failed." });
    }
